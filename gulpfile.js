...
// 监听任务
gulp.task('watch', ['js:dev'], function (cb) {
  // 文件列表
  const _files = [
  ...
  gulp.watch(conf.devPath + conf.filesPath.js,['js:dev']);
...
// 编译任务
gulp.task('build', ['startProd', 'js', 'wxss', 'wxml', 'json', 'images'], function () {
...
gulp.task('lint', () => {
  return gulp.src(['*.{js,json}', '**/*.{js,json}', '!node_modules/**', '!dist/**'],{base: './'+conf.devPath})
      .pipe(plugins.eslint())
